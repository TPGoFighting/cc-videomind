...
import { ArrowLeft, Heart, SlidersHorizontal, BarChart2, Check, Volume2, VolumeX, Trash2, X } from "lucide-react-native";
...
export default function WordsScreen() {
  ...
  const [words, setWords] = useStorageState<MockWord[]>("settings:my-words-list", []);
  const [favoriteOverrides, setFavoriteOverrides] = useStorageState<Record<string, boolean>>("words:favorite-overrides", {});
  ...
  const apiWords = useMemo(() => {
    return (vocabQuery.data ?? []).map((item) => ({
      ...
      isFavorite: favoriteOverrides[item.lemma.toLowerCase()] ?? false,
    }));
  }, [vocabQuery.data, favoriteOverrides]);
    ...
    apiWords.forEach((aw) => {
      ...
      if (existing) {
          mergedMap.set(key, {
            ...
            isFavorite: favoriteOverrides[key] ?? existing.isFavorite,
          });
      } else {
  ...
  }, [words, apiWords, favoriteOverrides]);

  const toggleFavoriteWord = (item: MockWord) => {
    haptics.selection();
    const key = item.lemma.toLowerCase();
    const nextFavorite = !item.isFavorite;
    setFavoriteOverrides((current) => ({ ...current, [key]: nextFavorite }));
    setWords((current) => current.map((word) => (
      word.lemma.toLowerCase() === key ? { ...word, isFavorite: nextFavorite } : word
    )));
  };
    ...
    Alert.alert("删除单词", `确定要从生词本中删除 "${item.lemma}" 吗？`, [
      { text: "取消", style: "cancel" },
      {
        ...
        onPress: async () => {
          ...
          try {
            // Remove locally
            setWords((current) => current.filter((w) => w.lemma.toLowerCase() !== item.lemma.toLowerCase()));
            ...
            Alert.alert("删除失败", e?.message || "删除失败，已恢复这条单词记录。");
          }
        },
      },
    ]);
  ...
  const filteredWords = useMemo(() => {
    ...
    // This filter describes its actual rule; it is not a language classifier.
    if (langFilter === "en100") {
      result = result.filter((w) => /^[A-Za-z]+$/.test(w.lemma) && w.lemma.length <= 5);
    }
      ...
      <SafeAreaView style={{ flex: 1 }} edges={["top"]}>
        ...
        <View style={styles.header}>
          ...
          <Pressable
            ...
            accessibilityRole="button"
            accessibilityLabel={showOnlyFavorites ? "显示全部单词" : "只显示收藏单词"}
        ...
        <FlatList
          data={filteredWords}
          keyExtractor={(item) => item.lemma.toLowerCase()}
          ...
          ListEmptyComponent={
            vocabQuery.isError ? (
              <EmptyState
                title="词库加载失败"
                description="网络请求没有完成，请重试后再继续学习。"
                actionLabel="重试"
                onAction={() => vocabQuery.refetch()}
              />
            ) : (
              <EmptyState
                title={showOnlyFavorites ? "暂无收藏的单词" : "单词本为空"}
                description={showOnlyFavorites ? "收藏字幕中的词语后，它们会出现在这里。" : "打开一个视频，从字幕中点选单词即可加入词库。"}
                actionLabel="返回视频列表"
          ...
          renderItem={({ item }) => (
            <Pressable
              ...
              accessibilityRole="button"
              accessibilityLabel={`打开单词详情：${item.lemma}`}
              style={({ pressed }) => [
                styles.wordRow,
                {
              ...
              <View style={styles.wordHeader}>
                <View style={{ flexDirection: "row", alignItems: "center", gap: 8, flex: 1, flexWrap: "wrap" }}>
                  ...
                  <Pressable
                    hitSlop={8}
                    accessibilityRole="button"
                    accessibilityLabel={`播放单词：${item.lemma}`}
                    onPress={(e) => {
                      e.stopPropagation();
                      haptics.light();
                ...
                <Pressable
                  onPress={(e) => {
                    e.stopPropagation();
                    toggleFavoriteWord(item);
                  }}
                  hitSlop={8}
                  style={styles.heartBtn}
                  accessibilityRole="button"
                  accessibilityLabel={item.isFavorite ? "取消收藏单词" : "收藏单词"}
                >
                  <Heart
                ...
                <Pressable
                  accessibilityRole="button"
                  accessibilityLabel={`删除单词 ${item.lemma}`}
                  onPress={(e) => {
                    e.stopPropagation();
                  ...
                  style={styles.heartBtn}
                >
                  <Trash2 color={theme.colors.subtle} size={18} />
                </Pressable>
              </View>
          ...
          >
            <View style={styles.overlay}>
              <Pressable
                ...
                accessibilityRole="button"
                accessibilityLabel="关闭筛选弹窗"
              />
              <View style={[styles.sheetContainer, { backgroundColor: theme.colors.surface, borderColor: theme.colors.border, paddingBottom: insets.bottom + 20 }]}>
                {/* Sheet Header */}
                ...
                <ScrollView contentContainerStyle={{ paddingHorizontal: 24, gap: 20 }}>
                  ...
                  <View style={{ gap: 8 }}>
                    ...
                    <Pressable
                      ...
                      accessibilityLabel="语言过滤：全部语言"
                      accessibilityState={{ selected: langFilter === "all" }}
                    ...
                    <Pressable
                      ...
                      accessibilityRole="radio"
                      accessibilityLabel="语言过滤：短单词"
                      accessibilityState={{ selected: langFilter === "en100" }}
                      style={[styles.filterOption, { backgroundColor: theme.colors.surfaceRaised, borderRadius: theme.radius.md }]}
                    >
                      <Text style={[styles.optionText, { color: theme.colors.text }]}>短单词（≤5 个字母）</Text>
                  ...
                  <View style={{ gap: 8, paddingBottom: 24 }}>
                    ...
                    {[
                      ...
                      <Pressable
                        ...
                        accessibilityRole="radio"
                        accessibilityLabel={`排序方式：${opt.label}`}
                        accessibilityState={{ selected: sortMethod === opt.key }}
                        style={[styles.filterOption, { backgroundColor: theme.colors.surfaceRaised, borderRadius: theme.radius.md }]}
                      >
