...
import { Modal, Pressable, Text, View, StyleSheet, TextInput, Alert, ActivityIndicator, KeyboardAvoidingView, Platform } from "react-native";
...
import { importedVideoTitle, parseVideoInput, userFacingError } from "@/lib/mobile-utils";
...
export default function TabLayout() {
  ...
  const handleUrlSubmit = async () => {
    ...
    if (!targetUrl) {
      setUrlError("请粘贴 YouTube、B站链接，或输入视频 ID。");
      return;
    }
    ...
    const res = parseVideoInput(targetUrl);
    if (!res) {
      haptics.error();
      const message = t("common.invalidUrl");
      setUrlError(message);
    ...
    const cleanTitle = importedVideoTitle(res);
    addParsingTask(res.id, cleanTitle, "parsing", 10);

    // Persist the id immediately so the media tab and feed can show an
    // imported video while transcript/analysis requests are still running.
    const currentMediaIds = storage.get<string[]>("settings:my-media-ids", []);
    if (!currentMediaIds.includes(res.id)) {
      storage.set("settings:my-media-ids", [res.id, ...currentMediaIds]);
    }
  ...
  const uploadAndParseLocalFile = async (uri: string, name: string, type: string, durationSeconds: number) => {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        let message = `服务器响应错误（HTTP ${response.status}）`;
        try {
          const parsed = JSON.parse(errText);
          message = parsed?.error?.message || parsed?.message || message;
        } catch {
          // Server may return a non-JSON error page; keep the HTTP status.
        }
      ...
      Alert.alert("导入解析失败", userFacingError(err, "网络请求失败，请检查后端服务是否开启。"));
    } finally {
    ...
    try {
      ...
      if (!result.canceled && result.assets?.length > 0) {
        ...
        await uploadAndParseLocalFile(
          ...
          Math.max(0, Math.round(Number((asset as { duration?: number }).duration) || 0))
        );
      }
    } catch (e: any) {
          ...
          <View style={styles.overlay}>
            ...
            <View style={[styles.sheetContainer, { backgroundColor: theme.colors.surface, borderColor: theme.colors.border, paddingBottom: insets.bottom + 20 }]}>
              ...
              <Pressable
                ...
                accessibilityLabel={t("create.importByUrl")}
                onPress={() => {
                  haptics.selection();
                  setIsCreateOpen(false);
                  setUrlInput("");
              ...
              <Pressable
                accessibilityRole="button"
                accessibilityLabel={t("create.importFromFile")}
              ...
              <Pressable
                accessibilityRole="button"
                accessibilityLabel={t("create.importFromAlbum")}
          ...
          >
            <View style={[styles.modalCard, { backgroundColor: theme.colors.glassRaised, borderColor: theme.colors.glassBorder }]}>
              ...
              <View style={{ flexDirection: "row", gap: 12, marginTop: 20 }}>
                ...
                <Pressable
                  accessibilityRole="button"
                  accessibilityLabel={t("create.startAnalysis")}
              ...
              {urlError ? (
                <Text
                  ...
                  style={{ color: theme.colors.danger, fontSize: 13, lineHeight: 19 }}
                >
                  {urlError}
                </Text>
              ) : null}
