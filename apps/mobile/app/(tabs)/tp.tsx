...
// Generate thoughtful default check-in history to populate GitHub grid nicely on first open
const getMockCheckinHistory = () => {
  const history = [];
  const today = new Date();
  const pastDays = [1, 2, 4, 7, 8, 11, 15, 18, 19, 21, 22, 25, 28, 30, 31, 35, 36, 42, 49, 50, 56, 63, 70, 77];
  for (const dayOffset of pastDays) {
    const d = new Date();
    d.setDate(today.getDate() - dayOffset);
    const dateStr = d.toISOString().split("T")[0];
    const count = Math.floor(Math.random() * 22) + 1;
...
export default function TpTabScreen() {
  ...
  const [streak, setStreak] = useStorageState<number>("tp:review-streak", 3);
      ...
      <SafeAreaView style={{ flex: 1 }} edges={["top"]}>
        ...
        <View style={styles.header}>
          ...
          <View style={styles.headerActions}>
            <Pressable style={[styles.actionButton, { backgroundColor: theme.colors.surfaceRaised }]} onPress={() => haptics.light()}>
        ...
        >
          ...
          >
            <View style={{ flex: 1, gap: 4 }}>
              ...
              <Text style={styles.dailySubtitle}>{dueCount} 个词等待您的定制复习</Text>
            </View>
            <View style={styles.dailyReward}>
              <Text style={styles.dailyRewardText}>+{20 + dueCount}</Text>
              <Sparkles color="#0A1A00" size={16} strokeWidth={2.5} />
            </View>
          </Pressable>
  ...
  return (
    ...
    >
      ...
      <View style={[styles.practiceArt, { backgroundColor: `${item.accent}12` }]}>
        {locked ? <Lock color={theme.colors.subtle} size={26} /> : PREVIEW_RENDERERS[item.id](item.accent)}
