...
import { ArrowLeft, Heart, Search, X, Video } from "lucide-react-native";
...
export default function PhrasesScreen() {
  ...
  const [phrases, setPhrases] = useStorageState<Phrase[]>("settings:my-phrases-list", []);
  const [showOnlyFavorites, setShowOnlyFavorites] = useState(false);
  const [searchQuery, setSearchQuery] = useState("");

  const toggleFavorite = (id: string) => {
    haptics.selection();
    setPhrases((current) => current.map((p) => (p.id === id ? { ...p, isFavorite: !p.isFavorite } : p)));
  };
    ...
    <Screen>
      <SafeAreaView style={{ flex: 1 }} edges={["top"]}>
        <View style={styles.header}>
          ...
          <Pressable
            onPress={() => { haptics.medium(); setShowOnlyFavorites((current) => !current); }}
            style={styles.backBtn}
            accessibilityRole="button"
            accessibilityLabel={showOnlyFavorites ? "显示全部短语" : "只显示收藏短语"}
        ...
        <FlatList
          ...
          ListEmptyComponent={
            <EmptyState
              title={showOnlyFavorites ? "暂无收藏的短语" : "暂无短语"}
              description="从视频字幕中收藏短语后，会显示在这里。"
              actionLabel="去视频列表"
            ...
            >
              <View style={{ flexDirection: "row", alignItems: "flex-start", gap: 10 }}>
                ...
                <Pressable
                  ...
                  accessibilityRole="button"
                  accessibilityLabel={item.isFavorite ? "取消收藏短语" : "收藏短语"}
                >
                  <Heart
...
const styles = StyleSheet.create({
  ...
  clearBtn: { width: 48, height: 48, alignItems: "center", justifyContent: "center" },
  inlineAction: { width: 48, height: 48, alignItems: "center", justifyContent: "center" },
