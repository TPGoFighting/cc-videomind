...
import { ArrowLeft, Heart, SlidersHorizontal, Volume2, VolumeX } from "lucide-react-native";
import { useTheme } from "@/providers/theme-provider";
import { useStorageState } from "@/hooks/use-storage-state";
import { useHaptics } from "@/hooks/use-haptics";
import { useSpeech } from "@/hooks/use-speech";
import { Screen } from "@/components/ui";
import { INITIAL_SENTENCES, type MockSentence } from "@/lib/mock-data";
...
export default function SentencesScreen() {
  ...
  const toggleFavorite = (id: string) => {
    haptics.selection();
    setSentences(
      sentences.map((s) => (s.id === id ? { ...s, isFavorite: !s.isFavorite } : s))
    );
  };

  // Merge online and local quotes seamlessly
  const mergedSentences = useMemo(() => {
    const localList = sentences ?? INITIAL_SENTENCES;
          ...
          ListEmptyComponent={
            <View style={styles.emptyContainer}>
              <Text style={[styles.emptyText, { color: theme.colors.muted }]}>
                {showOnlyFavorites ? "暂无收藏的句子" : "句子本为空，请从视频中收集句子"}
              </Text>
            </View>
          }
          renderItem={({ item }) => (
            ...
            >
              ...
              <View style={styles.cardFooter}>
                ...
                <Text style={[styles.source, { color: theme.colors.subtle, fontWeight: "700" }]} numberOfLines={1}>
