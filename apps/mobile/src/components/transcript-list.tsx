...
const DisplayModeToggle = memo(function DisplayModeToggle({
  ...
  return (
    <View style={{ flexDirection: "row", gap: 4, borderRadius: theme.radius.md, backgroundColor: theme.colors.surfaceRaised, padding: 3 }}>
      {options.map((opt) => {
        ...
        return (
          <Pressable
            ...
            accessibilityLabel={`Display ${opt.label}`}
...
const WordCapsuleButton = memo(function WordCapsuleButton({
  ...
  return (
    <Pressable
      onPress={() => { haptics.selection(); onWordPress(word); }}
      style={({ pressed }) => ({
        paddingHorizontal: 4,
        paddingVertical: 1,
        borderRadius: theme.radius.sm,
...
const TranscriptItem = memo(function TranscriptItem({
  ...
  const zhText = item.text_zh?.trim() || item.text;
  const awaitingTranslation = !item.text_zh?.trim() && showZh;
  ...
  return (
    ...
    >
        <View style={{ flexDirection: "row", alignItems: "center", justifyContent: "space-between" }}>
          <Text style={{ color: active ? theme.colors.accent : theme.colors.warm, fontSize: 12, fontWeight: "800", fontVariant: ["tabular-nums"] }}>
          ...
          <View style={{ flexDirection: "row", alignItems: "center", gap: 16 }}>
            <Pressable
              ...
              onPress={() => { haptics.light(); onGrammarPress(item); }}
              hitSlop={8}
            >
            ...
            <Pressable
              ...
              onPress={() => { haptics.selection(); onSharePress(item); }}
              hitSlop={8}
            >
            ...
            <Pressable
              ...
              onPress={() => { if (!saved && !saveMutation.isPending) { haptics.light(); saveMutation.mutate(); } }}
              hitSlop={8}
        ...
        {showZh && (displayMode !== "bilingual" || item.text_zh !== item.text || awaitingTranslation) ? (
          <View style={{ gap: 4 }}>
            {awaitingTranslation ? (
              <Text style={{ color: theme.colors.subtle, fontSize: 11, fontWeight: "700" }}>翻译中，先显示原文兜底</Text>
            ) : null}
            <Text selectable style={{ color: item.text_zh ? theme.colors.muted : theme.colors.subtle, fontSize: 14, lineHeight: 20 }}>
              {zhText}
            </Text>
          </View>
        ) : null}
      ...
      {sharingSegment && (
        <View style={{ position: "absolute", top: -10000, left: -10000 }}>
          <QuoteShareCard
            ...
            thumbnailUrl="https://images.unsplash.com/photo-1611162617213-7d7a39e9b1d7?w=500" // Mock or derive from video
            videoTitle="分享金句 - Teach Player"
          />
        </View>
      )}
    </View>
