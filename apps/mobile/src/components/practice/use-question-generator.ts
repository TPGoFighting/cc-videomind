...
import type { MockWord, MockSentence } from "@/lib/mock-data";
...
function pickDistractors(
  ...
  while (distractors.length < 2 && attempts < maxAttempts) {
    attempts++;
    const rWord = pool[Math.floor(Math.random() * pool.length)];
...
export function useQuestionGenerator({
  ...
  const generate = useCallback(() => {
    ...
    setTimeout(() => {
      try {
        if (moduleId === "words" || (moduleId === "mistakes" && mistakes.length === 0)) {
          const pool = vocabularyPool;
          const chosen = pool[Math.floor(Math.random() * pool.length)];
          const correctAns = cleanDefinition(chosen.definitionZh);
          ...
          while (distractors.length < 2 && attempts < 50) {
            attempts++;
            const rPhrase = pool[Math.floor(Math.random() * pool.length)].phrase;
          ...
          const chosen = sentencesPool[Math.floor(Math.random() * sentencesPool.length)];
          setQuestionData({
            sample: chosen.text,
        ...
        } else if (moduleId === "mistakes") {
          const chosen = mistakes[Math.floor(Math.random() * mistakes.length)];
          const correctAns = cleanDefinition(chosen.definitionZh);
          const distractors = pickDistractors(vocabularyPool, chosen, correctAns);
          setQuestionData({
            sample: chosen.lemma,
            correctAnswer: correctAns,
            choices: shuffleArray([correctAns, ...distractors]),
