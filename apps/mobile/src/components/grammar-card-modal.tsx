...
export function GrammarCardModal({
  ...
  const { data, isLoading, error, refetch, isFetching } = useQuery<GrammarAnalysisResult>({
    queryKey: ["grammar", videoId, sentence, accessToken],
      ...
      >
        ...
        >
          ...
          {isLoading ? (
            ...
            <View style={{ alignItems: "center", justifyContent: "center", paddingVertical: 40, gap: 14 }}>
              <Text style={{ color: theme.colors.danger, fontWeight: "600" }}>分析失败，请稍后重试</Text>
              <Pressable
                accessibilityRole="button"
                accessibilityLabel="重试语法分析"
                disabled={isFetching}
                onPress={() => refetch()}
                style={{ minHeight: 48, minWidth: 132, paddingHorizontal: 18, borderRadius: theme.radius.md, backgroundColor: theme.colors.accent, alignItems: "center", justifyContent: "center", opacity: isFetching ? 0.6 : 1 }}
              >
                {isFetching ? <ActivityIndicator color="#0A1A00" /> : <Text style={{ color: "#0A1A00", fontWeight: "900" }}>重新分析</Text>}
              </Pressable>
            </View>
          ) : data ? (
