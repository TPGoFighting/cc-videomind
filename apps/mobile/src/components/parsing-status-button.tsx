  ...
  return (
    <>
      <Pressable
        ...
        accessibilityLabel={hasActive ? `后台解析进度，有 ${activeTasks.length} 个任务进行中` : "查看后台解析进度"}
        accessibilityState={{ busy: hasActive }}
        onPress={handleOpen}
        style={[
          styles.button,
          {
                ...
                tasks.map(task => {
                  ...
                  return (
                    <Pressable
                      ...
                      accessibilityLabel={`${task.title}：${isUploading ? "正在上传" : isParsing ? "正在解析" : isFailed ? "解析失败" : "解析完成"}`}
                      accessibilityState={{ disabled: !isCompleted, busy: isUploading || isParsing }}
                      style={({ pressed }) => [
                        styles.taskRow,
                        {
                          borderColor: theme.colors.border,
