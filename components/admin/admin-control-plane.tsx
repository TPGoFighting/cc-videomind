        ...
        {snapshot && (
          <>
            <div className="grid gap-3 sm:grid-cols-3">
              <StatusTile icon={<Bot className="h-4 w-4" />} label="当前模型" value={draft.model || "未设置"} tone="blue" />
              <StatusTile icon={<ServerCog className="h-4 w-4" />} label="接口" value={draft.baseUrl || "未设置"} tone="slate" />
                ...
                <CardContent className="space-y-5">
                  ...
                  {Object.values(snapshot.config.envOverrides).some(Boolean) && (
                    <div className="flex gap-2 rounded-xl border border-amber-400/20 bg-amber-400/8 p-3 text-xs leading-5 text-amber-200/80">
                      ...
                      服务器存在环境变量覆盖项；环境变量优先于数据库设置。控制台已标出配置来源，避免保存后产生误判。
                    </div>
                  )}
...
function StatusTile({ icon, label, value, tone }: { icon: React.ReactNode; label: string; value: string; tone: "blue" | "slate" | "green" }) {
  const colors = {
