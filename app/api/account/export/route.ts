...
export async function GET(request: Request) {
  return withSecurity({
    ...
    const [
      ...
      queryTencent(`SELECT id, lemma, video_id, definition_zh, definition_en, phonetic, part_of_speech, example_en, example_zh, created_at, updated_at FROM user_vocabulary WHERE user_id = $1 ORDER BY created_at DESC`, [user.id]),
