...
const steps = [
  { step: "01", title: "粘贴链接", desc: "复制任意 YouTube 公开视频链接" },
  { step: "02", title: "AI 解析", desc: "自动提取字幕、元数据与核心观点" },
  { step: "03", title: "交互学习", desc: "阅读摘要、提问答疑、收藏笔记" },
];
  ...
  return (
    <div className="min-h-screen bg-black text-white overflow-hidden">
      ...
      <div className="hidden md:block">
          ...
          <div className="grid gap-12 lg:grid-cols-[1.05fr_0.95fr] lg:gap-16">
            <div className="flex flex-col justify-center space-y-8">
              ...
              <div className="space-y-4 animate-fade-in-up animate-fade-in-up-delay-1">
                <h1 className="heading-compressed max-w-2xl text-[28px] font-bold leading-[1.15] sm:text-[36px] md:text-[48px] lg:text-[64px] lg:leading-[1.06] tracking-tight">
                  把任何公开的 YouTube 课程
                  <br />
              ...
              <div className="animate-fade-in-up animate-fade-in-up-delay-2">
                ...
                <div className="mt-5">
                  <Link
                    href="/subscribe"
                    className="inline-flex items-center gap-2 rounded-full border border-[#0099ff]/30 bg-[#0099ff]/10 px-5 py-2 text-[14px] font-medium text-[#0099ff] transition-all hover:bg-[#0099ff]/20 hover:border-[#0099ff]/50 btn-press"
                  >
                ...
                {steps.map((s, i) => (
                  <div key={s.step} className="flex items-center gap-3">
                    <div className="flex items-center gap-2.5 rounded-lg border border-white/6 bg-white/[0.02] px-3.5 py-2.5 backdrop-blur-sm transition-colors hover:border-white/12 hover:bg-white/[0.04]">
                      <span className="font-mono text-[11px] font-semibold text-[#0099ff]/50 tabular-nums">{s.step}</span>
                      <span className="text-[13px] font-medium text-white/50">{s.title}</span>
                    </div>
                    {i < steps.length - 1 && <ArrowRight className="h-3.5 w-3.5 text-white/10 shrink-0" />}
                  </div>
                ))}
              </div>

              {/* 平板端操作步骤 */}
              <div className="flex sm:hidden flex-col gap-2 animate-fade-in-up animate-fade-in-up-delay-3">
                {steps.map((s) => (
                  <div key={s.step} className="flex items-center gap-3 rounded-lg border border-white/6 bg-white/[0.02] px-3.5 py-2.5">
                    <span className="font-mono text-[13px] font-semibold text-[#0099ff]/70 tabular-nums">{s.step}</span>
                    <div>
                      <span className="text-[13px] font-medium text-white/70">{s.title}</span>
                      <span className="text-[12px] text-white/35 ml-2">{s.desc}</span>
                    </div>
                  </div>
                ))}
              </div>
        ...
        {/* 特性网格 */}
        <section className="relative mx-auto w-full max-w-full px-4 py-16 sm:max-w-[90%] sm:px-5 sm:py-24 md:max-w-[85%] lg:max-w-[80%] pb-20 md:pb-16">
          <div className="mb-10 text-center space-y-3">
            <h2 className="text-[24px] font-bold tracking-tight sm:text-[30px]">不只是转录工具</h2>
          ...
          <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3 stagger-children">
            {features.map((item) => {
              ...
              return (
                <div
                  key={item.title}
                  className="card-lift group relative rounded-2xl border border-white/6 bg-[#0a0a0a] p-6 transition-all duration-500 hover:border-[#0099ff]/20 hover:bg-[#0d0d0d] hover:shadow-[rgba(0,153,255,0.06)_0px_0px_40px]"
                >
                  ...
                  <div className="relative">
                    ...
                    <h3 className="text-[15px] font-semibold tracking-tight transition-colors duration-300 group-hover:text-white/95">{item.title}</h3>
                    <p className="mt-2 text-[14px] leading-relaxed text-[#a6a6a6] transition-colors duration-300 group-hover:text-white/55">{item.body}</p>
                  </div>
                </div>
              );
            })}
        ...
        <footer className="relative border-t border-white/6 px-5 py-8 text-center">
          <p className="text-[13px] text-white/20">Teach Player · 基于真实转录的 AI 学习工具</p>
        </footer>
      </div>
...
function BottomCta() {
  return (
    <section className="relative mx-auto w-full max-w-full px-4 py-16 sm:max-w-[90%] sm:px-5 sm:py-24 md:max-w-[85%] lg:max-w-[80%]">
      <div className="relative overflow-hidden rounded-3xl border border-white/8 bg-gradient-to-br from-[#0a0f1a] via-[#0d0d0d] to-[#0a0f1a] px-6 py-12 sm:px-12 sm:py-16">
        {/* 背景光晕 */}
        <div className="pointer-events-none absolute -top-1/2 left-1/2 -translate-x-1/2 w-[600px] h-[600px] rounded-full bg-[#0099ff]/[0.04] blur-[100px]" />

        <div className="relative text-center space-y-6">
          <h2 className="text-[22px] font-bold tracking-tight sm:text-[32px]">
            准备好提升学习效率了吗？
          </h2>
          <p className="text-[15px] text-[#a6a6a6] max-w-md mx-auto">
            粘贴任意 YouTube 链接，秒级获取深度分析
          </p>
