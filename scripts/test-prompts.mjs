/**
 * AI Prompt 对比测试 —— 用 curl 调用 DeepSeek API
 * 运行: node scripts/test-prompts.mjs
 */
...
function getEnv(name) {
  const content = readFileSync(join(root, ".env.production.local"), "utf-8");
...
const API_KEY = getEnv("AI_API_KEY");
...
function transcriptStr(segments) {
...
function promptImproved() {
  const example = {
    ...
    takeaways: [
      ...
      "数据隐私是核心挑战：联邦学习允许多家医院在不共享原始数据的前提下共同训练模型",
      "算法偏见不容忽视：2019年研究发现某医疗算法对黑人患者存在系统性偏见",
      "数字孪生是未来方向：为每个病人创建AI虚拟副本，在虚拟环境中测试个性化治疗方案"
    ],
    suggestedQuestions: [
  ...
  return [
    ...
    "- 摘要要覆盖视频的主要话题和关键结论，有实质信息量",
    "- 每条要点必须包含具体的数据、案例、机构名或因果逻辑，避免空洞概括",
    "- 避免「AI是重要的技术」这类泛泛而谈，要写出「为什么重要、怎么做到的」",
    "- 要点之间应覆盖不同维度，不要重复",
    "- 亮点要选择真正有信息价值的内容片段",
    "",
...
// ====== 调用 DeepSeek API ======
async function callAPI(prompt, label) {
  ...
  writeFileSync(join(__dirname, "_req_body.json"), body, "utf-8");
  ...
  try {
    const result = execSync(
      `curl -s --connect-timeout 15 --max-time 120 -X POST "${BASE_URL}" -H "Content-Type: application/json" -H "Authorization: Bearer ${API_KEY}" --data-binary @${join(__dirname, "_req_body.json")}`,
    ...
    try {
      ...
      console.log(`⚠️ 无法解析 JSON，原始输出前 300 字:\n${raw.slice(0, 300)}`);
      return null;
    }
  } catch (e) {
...
function printResult(r) {
  ...
  r.takeaways?.forEach((t, i) => console.log(`   ${i + 1}. ${t}`));
  console.log(`\n❓ 问题 (${r.suggestedQuestions?.length ?? 0}条):`);
  r.suggestedQuestions?.forEach((q, i) => console.log(`   ${i + 1}. ${q}`));
  console.log(`\n🔦 亮点 (${r.highlights?.length ?? 0}条):`);
  r.highlights?.slice(0, 3).forEach((h, i) => console.log(`   ${i + 1}. [${h.startTime}s] ${h.title}`));
...
async function main() {
  ...
  console.log(`Key: ${API_KEY.slice(0, 5)}...${API_KEY.slice(-4)} (${API_KEY.length} chars)`);
  console.log(`字幕段数: ${TRANSCRIPT.length}`);

  // 测试当前 Prompt
  console.log(`\n\n🔬 测试 1/2: 当前 Prompt`);
  const r1 = await callAPI(promptCurrent(), "当前 Prompt");
  printResult(r1);

  // 测试改进版 Prompt
  console.log(`\n\n🔬 测试 2/2: 改进版 Prompt`);
  const r2 = await callAPI(promptImproved(), "改进版 Prompt");
  printResult(r2);
  ...
  if (r1 && r2) {
    console.log("\n📊 质量对比:");
    console.log(`   摘要长度: 当前=${r1.summary?.length ?? 0}字 vs 改进=${r2.summary?.length ?? 0}字`);
    console.log(`   要点数量: 当前=${r1.takeaways?.length ?? 0}条 vs 改进=${r2.takeaways?.length ?? 0}条`);
    console.log(`   亮点数量: 当前=${r1.highlights?.length ?? 0}条 vs 改进=${r2.highlights?.length ?? 0}条`);
  }
}
